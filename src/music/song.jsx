...
export default class Song extends Component {
  constructor(props) {
    super(props);
    let song = JSON.parse(localStorage.getItem("songs")).find(song => {
      return song.id == props.match.params.id;
    });
  ...
  onKeyPress(event) {
    ...
    let currentIndex = orderOfNotes.indexOf(
    ...
    switch (key) {
        ...
        this.setState({
          selected:
            this.state.selected == undefined
              ? 0
              : (this.state.selected + 1) % (this.state.notes.length + 1)
        });
        break;
      case "ArrowLeft":
        this.setState({
          selected:
            this.state.selected == undefined
              ...
              ? this.state.notes.length
              : (this.state.selected - 1) % this.state.notes.length
        });
        break;
      ...
      case "ArrowUp":
        this.updateNote(
          this.state.selected,
          orderOfNotes[
            currentIndex == orderOfNotes.length - 1
              ? orderOfNotes.length - 1
              : currentIndex + 1
          ]
        );
        break;
      case "ArrowDown":
        this.updateNote(
          this.state.selected,
          orderOfNotes[currentIndex == 0 ? 0 : currentIndex - 1]
        );
        break;
      case "Backspace":
      case "Delete":
    ...
    return (
      <React.Fragment>
        ...
        <div className="noteGroup">
          ...
          <AddNote
            selected={this.state.selected == this.state.notes.length}
            action={() => this.addNote("d")}
          />
        </div>
      </React.Fragment>
    );
